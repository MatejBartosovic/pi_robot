...
uint8_t computeCRC(uint8_t *buffer, unsigned int num){
        ...
        while((ind<num)){
        	MSG_CRC = msg_har_tableCRC[MSG_CRC ^ (*buffer++)];
            ind++;
        }
...
void sendOdom(float* data, uint8_t MSG_TYPE,UART_HandleTypeDef* uart){
	...
	msg.leftVel =  *(data+2);
	msg.rightVel =  *(data+3);
	msg.crc = computeCRC((uint8_t *)(&msg),sizeof(OutMsg)-1);
	HAL_UART_Transmit_DMA(uart,(uint8_t*)(&msg), sizeof(OutMsg));
